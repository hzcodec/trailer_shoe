...
module trailing_shoe_airplane_large_with_hole() {
    shoe_with_hole();
}
...
module base3_with_hole() {
    ...
    difference() {
        ...
        translate([5, 0, 0]) cylinder(h=2, r=3.5, center=true);
    }
}

module tip3_with_hole() {
    tip_length = 6;
    tip_width = width;
    tip_thickness = 2;
...
module shoe3_with_hole() {
    base3_with_hole();

    translate([-8, 0, 4]) pin_holder3_with_hole();

    color([1, 0, 0])
    translate([15.1, 0, 1.4]) rotate([0, -30, 0]) tip3_with_hole();
...
module shoe_with_hole() { 
    for (i = [0:number_of_items-1]) {
        translate([0, dist_between_objects*i, 0]) shoe3_with_hole();
    }
